...
public class CodecTest {
    ...
    public static void compress(String codecClassName) throws Exception{
        ...
        Configuration conf = HdfsConfig.config();
        FileSystem fs = FileSystem.get(conf);
        CompressionCodec codec = (CompressionCodec)ReflectionUtils.newInstance(codecClass, conf);
        //指定压缩文件路径
        FSDataOutputStream outputStream = fs.create(new Path(" /swordson@gmail.com/测试3.tar.gz"));
        //指定要被压缩的文件路径
        FSDataInputStream in = fs.open(new Path("/swordson@gmail.com/测试3"));
        //创建压缩输出流
